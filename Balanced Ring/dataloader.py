...
def estimate_epoch_duration(trainloader, world_size, model, device, num_batches=10):
    ...
    local_duration_array = np.array([local_duration], dtype=np.float64)
    all_durations_array = np.zeros(world_size, dtype=np.float64)

    # Gather durations from all nodes
    comm.Allgather([local_duration_array, MPI.DOUBLE], [all_durations_array, MPI.DOUBLE])

    # Calculate total duration
    total_duration = np.sum(all_durations_array)

    # Calculate efficiency ratios
    local_efficiency_ratio = local_duration / total_duration
    efficiency_ratios = [duration / total_duration for duration in all_durations_array]
