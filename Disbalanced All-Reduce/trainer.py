...
from communication import average_gradients_equal, average_gradients_weighted, average_weights_equal, average_weights_weighted

def train_global(model, trainloader, val_loader, trainset, valset, indices_train, indices_val, criterion, optimizer, scheduler, device, rank, world_size, num_local_epochs, num_global_epochs, timelimit, batch_size, prev_fraction, next_fraction, local_weight, fixed_classes, aggregation_type, aggregation_by, fixed_ratio):
    ...
    for global_epoch in progress_bar:
        ...
        for local_epoch in range(num_local_epochs):
            ...
            if rank == 0:  # Assuming we're interested in metrics from worker 0
        ...
        dist.all_reduce(epoch_duration_tensor, op=dist.ReduceOp.SUM)  # Sum durations across all nodes
