...
def main():
    ...
    # Loaders
    train_loader, val_loader, test_loader, trainset, valset, train_indices, val_indices, fixed_classes = get_loaders(args.batch_size, world_size, rank, model, device, args.fixed_ratio)
    ...
    all_workers_losses, all_epochs_losses, global_epoch_losses, global_epoch_accuracies, global_train_losses, global_train_accuracies, global_val_losses, global_val_accuracies, worker_specific_train_losses, worker_specific_train_accuracies, worker_specific_val_losses, worker_specific_val_accuracies = train_global(
        model, train_loader, val_loader, trainset, valset, train_indices, val_indices, criterion, optimizer, scheduler, device, rank, world_size, args.epochs_local, args.epochs_global, args.time_limit, args.batch_size, args.prev_fraction, args.next_fraction, args.local_weight, fixed_classes, args.aggregation_type, args.aggregation_by, args.fixed_ratio
    )
    ...
    # Visualization and evaluation (only on rank 0 for simplicity)
    if rank == 0:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--fixed_ratio', type=float, default=0.5, help='Portion of data to be from disbalanced classes')
