...
from communication import ring_all_reduce_equal, ring_all_reduce_weighted


def train_global(model, trainloader, val_loader, trainset, valset, indices_train, indices_val, criterion, optimizer, scheduler, device, rank, world_size, num_local_epochs, num_global_epochs, timelimit, batch_size, prev_fraction, next_fraction, comm, local_weight, fixed_classes, aggregation_type, aggregation_by, fixed_ratio):
    ...
    for global_epoch in progress_bar:
            ...
            if aggregation_type == 'weighted':
                for param in model.parameters():
                    if param.grad is not None:
                        ...
                        ring_all_reduce_weighted(param.grad.data, rank, world_size, local_weight)
            elif aggregation_type == 'equal':
                for param in model.parameters():
                    if param.grad is not None:
                        # Equal gradient aggregation
                        ring_all_reduce_equal(param.grad.data, rank, world_size)
        if aggregation_by == 'weights':
            if aggregation_type == 'weighted':
                for param in model.parameters():
                    if param.requires_grad:
                        # Weighed model weights aggregation
                        ring_all_reduce_weighted(param.data, rank, world_size, local_weight)
            elif aggregation_type == 'equal':
                for param in model.parameters():
                    if param.requires_grad:
                        # Equal model weights aggregation
                        ring_all_reduce_equal(param.data, rank, world_size)
        ...
        trainloader, val_loader, indices_train, indices_val = get_subset_loaders(trainset, valset, indices_train, indices_val, batch_size=batch_size, prev_fraction=prev_fraction, next_fraction=next_fraction, epoch_duration=epoch_duration, epoch_duration_tensor=epoch_duration_tensor, fixed_classes=fixed_classes, fixed_ratio=fixed_ratio)
